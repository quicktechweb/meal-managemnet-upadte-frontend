...
const UpdateAppSection = () => {
  ...
  const singleAppData = data?.find((item) => item?._id === id);

  const { mutateAsync, isPending } = useUpdateAppData();
  ...
  useEffect(() => {
    if (singleAppData) {
      reset({
        title: singleAppData?.title,
        description: singleAppData?.description,

        play_store_url: singleAppData?.play_store_url,
        apple_store_url: singleAppData?.apple_store_url,
      });
      setSelectedImage(singleAppData.bg_app || null);
      setSelectedbannerImage(singleAppData.img_app || null);
    }
  }, []);
  ...
  const onSubmit = async (data) => {
    ...
    formdata.append(
      "bg_app",
      selectedImagefile ? selectedImagefile : singleAppData.bg_app,
    );

    formdata.append(
      "img_app",
      selectedbannerImagefile ? selectedbannerImagefile : singleAppData.img_app,
    );
...
export default UpdateAppSection;
