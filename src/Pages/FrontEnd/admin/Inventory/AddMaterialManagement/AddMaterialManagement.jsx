...
export default function RawMaterialManager() {
  ...
  const loadCostings = useCallback(async () => {
    try {
      const { data } = await apiFetch("/costings");
      setCostings(data);
    } catch (e) {
      setError(e.message);
    }
  ...
  const loadSummary = useCallback(async () => {
    try {
      const res = await apiFetch("/costings/summary");
      setSummary(res);
    } catch (e) {
      setError(e.message);
    }
  ...
  const addOrUpdateMaterial = async () => {
    ...
    try {
      if (editId) {
        await apiFetch(`/${editId}`, { method: "PUT", body: JSON.stringify(body) });
        setEditId(null);
      } else {
  ...
  const addCosting = async () => {
    ...
    try {
      await apiFetch("/costings", { method: "POST", body: JSON.stringify(body) });
  ...
  const deleteCosting = async (id) => {
    ...
    try {
      await apiFetch(`/costings/${id}`, { method: "DELETE" });
      await loadCostings();
      await loadMaterials();
    } catch (e) { alert(e.message); }
  };
...
function PricePerGramBadge({ pricePerGram, unit }) {
  const nonWeightUnits = ["piece", "dozen", "bag", "packet"];
  if (nonWeightUnits.includes(unit)) {
  ...
  // Show per-ml label for liquid units
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-gray-500 border-b border-gray-100">
                  ...
                  <Th>
                    <span className="bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded text-xs">প্রতি গ্রাম/ml</span>
                  </Th>
                  <Th></Th>
                </tr>
              </thead>
                ...
                {materials.map((m) => {
                  ...
                  return (
                    <tr key={m._id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <Td>
                        <PricePerGramBadge pricePerGram={m.pricePerGram} unit={m.unit} />
                      </Td>
                      <Td>
